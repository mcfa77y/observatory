...
trait InteractionTest extends FunSuite with Checkers {
  ...
  test("make proper interaction image KD") {
    ...
    val temps_kd = KDTreeMap.fromSeq(temps.collect().toSeq)
    val image = Interaction.tile_KD(temps_kd, color_scale, tile)

    val zonedDateTimeIst = ZonedDateTime.now()
    val name = "KD_spagetti_"+zonedDateTimeIst.getHour.toString +"_"+zonedDateTimeIst.getMinute.toString + ".png"
  ...
  test("make interaction image KD") {
    val tile = Tile(0,0,0)
    ...
    val temps_KD = KDTreeMap.fromSeq(temps)
