...
object Visualization extends SparkJob {
  ...
  def dist_KM(j: Location, k: Location): Double = {
    ...
    if (j.eq(k)) {
      ...
      //c = cheap_acos(cheap_sin(theta1) * cheap_sin(theta2) + cheap_cos(theta1) * cheap_cos(theta2) * cheap_cos(abs(lambda1 - lambda2)))
  ...
  val COS_ANS = for (deg <- 0 to 360) yield cos(deg.toRadians)
  val ACOS_ANS = for (x <- -100 to 100) yield acos(x / 100d)
